function data = loadSheddingDurationData()
...
[numData,~,allData]=xlsread('sheddingDurationCDF.xlsx');

%% structure data
allData=allData(1:187,:); % for some reason, data is loading lots of nan rows
excludeIdx=strcmp(allData(:,48),'yes'); % remove studies excluded from analysis
allData=allData(~excludeIdx,:);
numData=numData(~excludeIdx,:);

% extract useful arrays
data.daysSinceChallenge=numData(1,2:36);
data.sampleSize=numData(2:end,1);
data.sheddingCDF=numData(2:end,2:36);
for k=1:length(data.sampleSize)
    data.sheddingCDF(k,:)=1-data.sheddingCDF(k,:)/data.sampleSize(k);
end

data.ageLastRIMonths=round(numData(2:end,37)*12/365);
data.challengeAgeMonths=round(numData(2:end,38)*12/365);
data.challengeSerotype=numData(2:end,39);
data.challengeDose=allData(2:end,47);

data.primaryImmunization=allData(2:end,45);
data.immunizationSchedule=allData(2:end,46);
data.publicationDate=numData(2:end,42);
data.location=allData(2:end,43);
data.author=allData(2:end,41);

% average by category

data.immunizationPlans=unique(data.primaryImmunization);
data.meanSheddingCDF=nan(length(data.immunizationPlans)*3,length(data.daysSinceChallenge));
...
for k=1:length(data.immunizationPlans)
    for n=1:3
        ...
        if any(idx)
            ...
            data.meanSheddingCDF(3*(k-1)+n,:)=tmp2;
